import type { ScraperSource } from "./types";

export const scraperSources: ScraperSource[] = [
  {
    scrapingApi: false,
    id: "stepstone-chinese-de",
    name: "StepStone DE - Chinese Keyword",
    nameZh: "StepStone 德国 - 中文关键词",
    type: "rss",
    url: "https://www.stepstone.de/jobs/chinesisch/rss",
    enabled: false,
    keywords: ["chinesisch", "chinese", "mandarin", "中文", "china", "china market", "chinese speaking"],
    defaultField: undefined,
    defaultLocationCode: "de",
  },
  {
    scrapingApi: true,
    id: "indeed-chinese-de",
    name: "Indeed DE - Chinese Keyword",
    nameZh: "Indeed 德国 - 中文关键词",
    type: "rss",
    url: "https://de.indeed.com/rss?q=chinesisch&l=Deutschland",
    enabled: false,
    keywords: ["chinesisch", "chinese", "mandarin", "中文", "china", "china market", "chinese speaking"],
    defaultLocationCode: "de",
  },
  {
    id: "adzuna-chinese-de",
    name: "Adzuna DE - Chinese Keyword",
    nameZh: "Adzuna 德国 - 中文关键词",
    type: "json-api",
    url: "https://api.adzuna.com/v1/api/jobs/de/search/1?app_id=demo&app_key=demo&results_per_page=50&what=chinesisch&where=Germany",
    enabled: true,
    keywords: ["chinesisch", "chinese", "mandarin"],
    defaultLocationCode: "de",
  },
  {
    id: "jobware-chinese-de",
    name: "Jobware DE - Chinese Keyword",
    nameZh: "Jobware 德国 - 中文关键词",
    type: "html",
    url: "https://www.jobware.de/jobs?search=chinesisch",
    enabled: true,
    keywords: ["chinesisch", "chinese", "mandarin"],
    selectors: {
      jobCard: ".job-card, .job-listing, article",
      title: "h2 a, h3 a, .job-title",
      company: ".company-name, .company",
      location: ".location, .job-location",
      link: "h2 a, h3 a, a[href*='/job/']",
    },
    defaultLocationCode: "de",
  },
  {
    scrapingApi: true,
    id: "bosch-careers-china",
    name: "Bosch Careers - China/AI",
    nameZh: "博世招聘 - 中国/AI",
    type: "html",
    url: "https://www.bosch.de/de/karriere/job-suche/?country=DE&keyword=chinese",
    enabled: true,
    jsRendered: true,
    puppeteerOptions: {
      waitForSelector: ".job-item, .job-result, .posting",
      waitTimeout: 12000,
      scrollDelay: 2000,
      extraWaitMs: 2000,
    },
    keywords: ["chinese", "chinesisch", "china", "mandarin", "china market", "chinese speaking"],
    selectors: {
      jobCard: ".job-item, .job-result, .posting",
      title: ".job-title, h3, h2",
      company: "Robert Bosch",
      location: ".job-location, .location",
      link: "a.job-link, a",
    },
    defaultField: "ai",
    defaultLocationCode: "de",
  },
  {
    id: "linkedin-chinese-de",
    name: "LinkedIn - Chinese Jobs Germany",
    nameZh: "LinkedIn - 德国中文职位",
    type: "html",
    url: "https://www.linkedin.com/jobs/search/?keywords=chinese%20language&location=Germany",
    enabled: false,
    scrapingApi: true, // high-risk: JS-rendered + anti-bot => try managed Scraping API first (requires SCRAPING_API_KEY)
    jsRendered: true,
    puppeteerOptions: {
      waitForSelector: ".jobs-search__results-list li, .job-search-card",
      waitTimeout: 15000,
      scrollDelay: 3000,
      extraWaitMs: 3000,
    },
    keywords: ["chinese", "chinesisch", "mandarin", "中文", "china", "china market", "chinese speaking"],
    selectors: {
      jobCard: ".jobs-search__results-list li, .job-search-card",
      title: ".base-search-card__title",
      company: ".base-search-card__subtitle",
      location: ".job-search-card__location",
      link: "a.base-card__full-link",
    },
    defaultLocationCode: "de",
  },
  {
    id: "xing-chinese-de",
    name: "XING Jobs - Chinese Keyword",
    nameZh: "XING 职位 - 中文关键词",
    type: "html",
    url: "https://www.xing.com/jobs/search?q=chinesisch&location=Deutschland",
    enabled: false,
    scrapingApi: true, // high-risk: JS-rendered + anti-bot => try managed Scraping API first (requires SCRAPING_API_KEY)
    jsRendered: true,
    puppeteerOptions: {
      waitForSelector: "[data-testid='job-posting-card'], .job-posting",
      waitTimeout: 12000,
      scrollDelay: 2000,
      extraWaitMs: 2000,
    },
    keywords: ["chinesisch", "chinese", "mandarin", "china", "china market", "chinese speaking"],
    selectors: {
      jobCard: "[data-testid='job-posting-card'], .job-posting",
      title: "h3 a, h4 a, [data-testid='job-title']",
      company: "[data-testid='company-name'], .company-name",
      location: "[data-testid='job-location'], .location",
      link: "h3 a, h4 a, a[href*='/jobs/']",
    },
    defaultLocationCode: "de",
  },
  // scrapingApi not enabled — direct/puppeteer sufficient for this source
  {
    id: "sapprosoftmoms-china",
    name: "SAP Careers - Chinese",
    nameZh: "SAP 招聘 - 中文",
    type: "html",
    url: "https://jobs.sap.com/search/?q=chinese&locationsearch=Germany",
    enabled: true,
    keywords: ["chinese", "chinesisch", "mandarin", "china", "china market", "chinese speaking"],
    selectors: {
      jobCard: ".job-result, .search-result",
      title: ".job-title, h3 a",
      company: "SAP SE",
      location: ".job-location",
      link: "h3 a, a.job-link",
    },
    defaultField: "cs",
    defaultLocationCode: "de",
  },
  {
    scrapingApi: true,
    id: "huawei-europe-china",
    name: "Huawei Europe Careers",
    nameZh: "华为欧洲招聘",
    type: "html",
    url: "https://career.huawei.com/reccampportal/portal5/campus-recruitment.html?jobType=2&language=en",
    enabled: true,
    jsRendered: true,
    puppeteerOptions: {
      waitForSelector: ".job-item, .position-item",
      waitTimeout: 15000,
      scrollDelay: 3000,
      extraWaitMs: 3000,
    },
    keywords: ["chinese", "mandarin", "中文", "china", "china market", "chinese speaking"],
    selectors: {
      jobCard: ".job-item, .position-item",
      title: ".job-title, .position-name",
      company: "Huawei",
      location: ".job-location",
      link: "a.job-link, a",
    },
    defaultLocationCode: "de",
  },
  {
    scrapingApi: true,
    id: "dfki-ai-china",
    name: "DFKI - AI Research",
    nameZh: "德国人工智能研究中心",
    type: "html",
    url: "https://www.dfki.de/en/jobs-and-careers/",
    enabled: true,
    keywords: ["chinese", "china", "ai", "machine learning", "robotics", "china market", "chinese speaking"],
    selectors: {
      jobCard: ".job-item, .vacancy, .position",
      title: "h3, h4, .job-title",
      company: "DFKI",
      location: ".location",
      link: "a",
    },
    defaultField: "ai",
    defaultLocationCode: "de",
  },
  {
    scrapingApi: true,
    id: "fraunhofer-ai-china",
    name: "Fraunhofer - AI/Robotics",
    nameZh: "弗劳恩霍夫 - AI/机器人",
    type: "html",
    url: "https://www.fraunhofer.de/en/jobs.html",
    enabled: true,
    keywords: ["chinese", "china", "ai", "robotics", "machine learning", "china market", "chinese speaking"],
    selectors: {
      jobCard: ".job-item, .vacancy, .search-result",
      title: "h3 a, h4 a, .job-title",
      company: "Fraunhofer",
      location: ".location",
      link: "a",
    },
    defaultField: "ai",
    defaultLocationCode: "de",
  },
  // scrapingApi not enabled — direct/puppeteer sufficient for this source
  {
    id: "make-it-in-germany",
    name: "Make it in Germany - IT/Chinese",
    nameZh: "在德国工作 - IT/中文",
    type: "html",
    url: "https://www.make-it-in-germany.com/en/working-in-germany/job-listings",
    enabled: true,
    keywords: ["chinese", "chinesisch", "ai", "robotics", "software", "china", "china market", "chinese speaking"],
    selectors: {
      jobCard: ".job-item, .job-result",
      title: "h3 a, h4 a, .job-title",
      company: ".company",
      location: ".location",
      link: "a",
    },
    defaultLocationCode: "de",
  },
  {
    scrapingApi: true,
    id: "machinelearningjobs-de",
    name: "MachineLearningJobs.de",
    nameZh: "机器学习职位德国",
    type: "html",
    url: "https://machinelearningjobs.de/jobs/",
    enabled: true,
    keywords: ["chinese", "chinesisch", "mandarin", "china", "china market", "chinese speaking"],
    selectors: {
      jobCard: ".job-listing, .job-card, article",
      title: "h2 a, h3 a, .job-title",
      company: ".company-name, .employer",
      location: ".location",
      link: "h2 a, h3 a, a.job-link",
    },
    defaultField: "ai",
    defaultLocationCode: "de",
  },
  // scrapingApi not enabled — direct/puppeteer sufficient for this source
  {
    id: "remoteok-chinese",
    name: "RemoteOK - Chinese",
    nameZh: "RemoteOK - 中文",
    type: "json-api",
    url: "https://remoteok.com/api?tags=chinese",
    enabled: true,
    keywords: ["chinese", "mandarin", "中文", "china", "china market", "chinese speaking"],
    defaultField: "remote",
    defaultLocationCode: "remote",
  },
  {
    scrapingApi: true,
    id: "dronejobs-de",
    name: "DroneJobs.de",
    nameZh: "无人机职位德国",
    type: "html",
    url: "https://www.dronejobs.de/jobs/",
    enabled: true,
    keywords: ["chinese", "chinesisch", "china", "china market", "chinese speaking"],
    selectors: {
      jobCard: ".job-item, .job-card, article",
      title: "h2 a, h3 a, .job-title",
      company: ".company-name",
      location: ".location",
      link: "h2 a, h3 a",
    },
    defaultField: "drone",
    defaultLocationCode: "de",
  },
  {
    id: "google-jobs-searchapi",
    name: "Google Jobs via SearchAPI",
    nameZh: "Google 职位 - SearchAPI",
    type: "json-api",
    url: "https://www.searchapi.io/api/v1/search",
    enabled: true,
    keywords: ["chinesisch", "chinese", "mandarin", "china", "china market", "chinese speaking"],
    defaultLocationCode: "de",
  },
];

export function getEnabledSources(): ScraperSource[] {
  return scraperSources.filter((s) => s.enabled);
}

export function getSourceById(id: string): ScraperSource | undefined {
  return scraperSources.find((s) => s.id === id);
}
